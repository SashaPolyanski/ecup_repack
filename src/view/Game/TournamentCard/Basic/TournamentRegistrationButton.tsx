...
export const TournamentRegistrationButtonComponent: FC<
  TournamentRegistrationButtonProps
> = ({ gamePk, tournamentId, status }) => {
  ...
  >({
    ...
    token: true,
    queryKeyRefetch: [
      `/games/${gamePk}/tournaments/${tournamentId}/teams/?team__users=${user?.id}`,
      `/games/${gamePk}/tournaments/`,
    ],
  });

  const { data: userTeam } = useQuery<PaginatedTournamentTeamReadOnlyList>({
    path: `/games/${gamePk}/tournaments/${tournamentId}/teams/?team__users=${user?.id}`,
  });
  const { mutate: unRegistered } = useMutation({
    path: `/games/${gamePk}/tournaments/${tournamentId}/teams/${
      userTeam?.results && userTeam?.results[0]?.id
    }`,
    method: "DELETE",
    token: true,
    queryKeyRefetch: [
  ...
  return (
    <TournamentRegistrationButtonContainer ml={2}>
      {!inTournament && status === "NOT_STARTED" ? (
        ...
        <>
          <Typography mr={1}>{t("youParticipate")}</Typography>
          <Button
            variant={"outlined"}
            onClick={unRegisterInTournamentHandler}
            loading={loading}
          >
            Отмена
          </Button>
...
export const TournamentRegistrationButton: FC<TournamentProps> = withGamePk()(
