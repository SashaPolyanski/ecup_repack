...
const ShowMoreIcon = styled(KeyboardArrowDownIcon, transientOptions)<{
  $collapsed: boolean;
}>`
...
const DrawerContentContainer = styled(Box)`
  ...
  margin-top: 20px;
`;
const MobileMenuItemWrapper = styled(Box, transientOptions)<{
  $collapsed?: boolean;
}>`
  display: flex;
  align-items: center;
...
const renderMobileMenuItems = (
  ...
  return items ? (
    items?.map(({ avatar, id, name }) => (
      <MobileMenuItemWrapper
        key={id}
        $collapsed={collapsed}
        onClick={navigateToGameHandler(id.toString())}
      >
...
export const MobileHeaderContent: FC<MobileHeaderContentProps> = ({
  onClose,
  open,
}) => {
  ...
  const modifyData = data?.results?.reduce(
    (acc, cur, i) => {
      if (i < 1) {
