...
const TournamentTabInfoComponentContainer = styled(Box)`
  ...
  width: 100%;
`;
export const TournamentTabInfoComponent: FC<TournamentTabInfoProps> = ({
  tournamentPk,
  gamePk,
}) => {
  ...
  const tournamentStartDate =
    ...
    new Date(data?.start_at)
      ...
      .slice(0, 10);
  const date = `${tournamentStartDate} ${data?.start_at?.slice(
    11,
    16,
  )} по Москве (GMT+3)`;
  return (
...
export const TournamentTabInfo = withTournamentPk()(
  withGamePk()(TournamentTabInfoComponent),
);
