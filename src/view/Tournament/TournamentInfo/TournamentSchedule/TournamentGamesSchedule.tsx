...
export const TournamentGameScheduleComponent: FC<TournamentScheduleProps> = ({tournamentPk, gamePk}) => {
  const {data} = useQuery<TournamentReadOnly>({
    path: `/games/${gamePk}/tournaments/${tournamentPk}/`,
  });
  return (
    <TournamentStepper schedule={data?.stage_schedule}/>
  );
};
export const TournamentGameSchedule = withTournamentPk()(withGamePk()(TournamentGameScheduleComponent))
